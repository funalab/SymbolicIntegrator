...
import pickle
from train_visualize_all_layer_decoder import source_pad_concat_convert
...
class MultiHeadAttention(chainer.Chain):
    ...
    def __call__(self, x, z=None, mask=None):
        ...
        batch_A = F.softmax(batch_A, axis=2)  ## ここがsoftmaxされた結果
        
        # self.attention_weight = np.asnumpy(batch_A._data[0]) if you use gpu
        self.attention_weight = batch_A._data[0] # if you don't use gpu
    ...
    def get_attention_weight(self):
        if self.attention_weight is not None:
            return self.attention_weight.copy()
        else:
...
class EncoderLayer(chainer.Chain):
    def __init__(self, n_units, h=8, dropout=0.1):
        ...
        self.dropout = dropout
        self.get_attention_weight = self.self_attention.get_attention_weight()
    ...
    def get_attention_weight(self):
        self.get_attention_weight = self.self_attention.get_attention_weight()
        return self.get_attention_weight
...
class DecoderLayer(chainer.Chain):
    ...
    def __call__(self, e, s, xy_mask, yy_mask):
        ...
        self.get_attention_weight = self.source_attention.get_attention_weight()
...
class Encoder(chainer.Chain):
    def __init__(self, n_layers, n_units, h=8, dropout=0.1):
        ...
        for i in range(1, n_layers + 1):
            ...
            self.Encoder_self_attention_weight_one_equation=[]
            self.attention_weight_encoder_self_attention = []
    def __call__(self, e, xx_mask):
        for name in self.layer_names:
            e = getattr(self, name)(e, xx_mask)
            #if name == 'l5':
            #    self.attention_weight_encoder_self_attention = getattr(self.l5, "get_attention_weight")
            self.attention_weight_encoder_self_attention.append(getattr(getattr(self, name), "get_attention_weight"))
        return e
...
class Decoder(chainer.Chain):
    def __init__(self, n_layers, n_units, h=8, dropout=0.1):
        ...
        for i in range(1, n_layers + 1):
            ...
            self.add_link(name, layer)
            self.layer_names.append(name)
            self.attention_weight_decoder_source_attention = []
    def __call__(self, e, source, xy_mask, yy_mask):
        for name in self.layer_names:
            e = getattr(self, name)(e, source, xy_mask, yy_mask)
            self.attention_weight_decoder_source_attention.append(getattr(getattr(self, name), "get_attention_weight"))
        return e
...
class Transformer(chainer.Chain):
    ...
    def __init__(self, n_layers, n_source_vocab, n_target_vocab, n_units,
        ...
        self.one_equation_attention_weight_l1 = []
        self.one_equation_attention_weight_l2 = []
        self.one_equation_attention_weight_l3 = []
        self.one_equation_attention_weight_l4 = []
        self.one_equation_attention_weight_l5 = []
        self.one_equation_attention_weight_l6 = []
        self.one_equation_attention_weight_decoder_l1 = [] 
        self.one_equation_attention_weight_decoder_l2 = [] 
        self.one_equation_attention_weight_decoder_l3 = [] 
        self.one_equation_attention_weight_decoder_l4 = [] 
        self.one_equation_attention_weight_decoder_l5 = [] 
        self.one_equation_attention_weight_decoder_l6 = [] 
    ...
    def __call__(self, x_block, y_in_block, y_out_block, get_prediction=False):
        ...
        self.one_equation_attention_weight_l1 = self.encoder.attention_weight_encoder_self_attention[0].copy()
        self.one_equation_attention_weight_l2 = self.encoder.attention_weight_encoder_self_attention[1].copy()
        self.one_equation_attention_weight_l3 = self.encoder.attention_weight_encoder_self_attention[2].copy()
        self.one_equation_attention_weight_l4 = self.encoder.attention_weight_encoder_self_attention[3].copy()
        self.one_equation_attention_weight_l5 = self.encoder.attention_weight_encoder_self_attention[4].copy()
        self.one_equation_attention_weight_l6 = self.encoder.attention_weight_encoder_self_attention[5].copy()
        
        self.encoder.attention_weight_encoder_self_attention.clear()
        # Encode Targets with Sources (Decode without Output)
        h_block = self.decoder(ey_block, z_blocks, xy_mask, yy_mask)

        self.one_equation_attention_weight_decoder_l1 = self.decoder.attention_weight_decoder_source_attention[0].copy()
        self.one_equation_attention_weight_decoder_l2 = self.decoder.attention_weight_decoder_source_attention[1].copy()
        self.one_equation_attention_weight_decoder_l3 = self.decoder.attention_weight_decoder_source_attention[2].copy()
        self.one_equation_attention_weight_decoder_l4 = self.decoder.attention_weight_decoder_source_attention[3].copy()
        self.one_equation_attention_weight_decoder_l5 = self.decoder.attention_weight_decoder_source_attention[4].copy()
        self.one_equation_attention_weight_decoder_l6 = self.decoder.attention_weight_decoder_source_attention[5].copy()
        
        self.decoder.attention_weight_decoder_source_attention.clear()
