...
def generate_model(trial):
    ...
    global n_out
    n_out= len(data.vocab)
    n_hidden = trial.suggest_int("n_hidden", 813, 813)
    n_layer = trial.suggest_int("n_layer", 5, 5)
    dropout = trial.suggest_uniform('dropout', 0.04037125968196613, 0.04037125968196613)
...
def create_optimizer(trial,model):
    ...
    if optimizer_name == 'Adam':
        adam_alpha = trial.suggest_loguniform('adam_alpha', 0.000552734718175031, 0.000552734718175031)
    ...
    weight_decay = trial.suggest_loguniform('weight_decay', 0.0008172597055154687, 0.0008172597055154687)
    print(f"    weight decay : {weight_decay}")
    gradient_clipping = trial.suggest_uniform('gradient_clipping', 8.208591704560082, 8.208591704560082)
...
def main():
    ...
    parser.add_argument('--Integrand_dataset', '-i',type=str, default='../dataset/LSTM_string_IRPP_train_valid_Integrand_first_10eq.txt')
    parser.add_argument('--Primitive_dataset', '-p',type=str, default='../dataset/LSTM_string_IRPP_train_valid_Primitive_first_10eq.txt')
    parser.add_argument('--study_name', '-s',type=str, default='MLP_cupy_MedianPruner_epoch30_integrand_reverse_polish_Primitive_polish_third_try_memory_edited_v102_continue_untilepoch200_new')
    parser.add_argument('--learned_model', '-m',type=str, default='../model/LSTM_string_IRPP_best_model_new')
