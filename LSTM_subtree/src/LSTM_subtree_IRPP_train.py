...
def generate_model(trial):
    ...
    global n_out
    n_out= len(data.vocab)
    n_hidden = trial.suggest_int("n_hidden", 1022, 1022)
    n_layer = trial.suggest_int("n_layer", 3, 3)
    dropout = trial.suggest_uniform('dropout', 0.19741943013106578, 0.19741943013106578)
...
def create_optimizer(trial,model):
    ...
    if optimizer_name == 'Adam':
        adam_alpha = trial.suggest_loguniform('adam_alpha', 9.767735830647477e-05, 9.767735830647477e-05)
    ...
    weight_decay = trial.suggest_loguniform('weight_decay', 5.639143534968458e-10, 5.639143534968458e-10)
    print(f"    weight decay : {weight_decay}")
    gradient_clipping = trial.suggest_uniform('gradient_clipping', 4.532449283887844, 4.532449283887844,)
...
def main():
    ...
    parser.add_argument('--token_dataset', '-T',type=str, default='../dataset/LSTM_subtree_IRPP_token.txt')
    parser.add_argument('--Integrand_dataset', '-i',type=str, default='../dataset/LSTM_subtree_IRPP_train_valid_Integrand_first_10eq.txt')
    parser.add_argument('--Primitive_dataset', '-p',type=str, default='../dataset/LSTM_subtree_IRPP_train_valid_Primitive_first_10eq.txt')
    parser.add_argument('--study_name', '-s',type=str, default='MLP_cupy_MedianPruner_epoch30_subtree_Integrand_reverse_polish_Primitive_polish_continue_new')
    parser.add_argument('--learned_model', '-m',type=str, default='../model/LSTM_subtree_IRPP_best_model_new')
